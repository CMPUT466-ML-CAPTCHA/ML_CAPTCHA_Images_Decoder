...
def update_matrix(pred, label, conf_matrix):
    for p, t in zip(pred, label):
        conf_matrix[TABLE.index(p), TABLE.index(t)] += 1
    return conf_matrix


def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix For KNN', cmap=plt.cm.Blues):
...
print("# neighbors = {}, Accuracy: {:.2f}%".format(20, accuracy*100))

print(len(y_predict))
assert len(y_predict) == len(test_y)

conf_matrix = torch.zeros(36, 36)

for i in range(len(y_predict)):
    conf_matrix = update_matrix(y_predict[i], test_y[i], conf_matrix)
